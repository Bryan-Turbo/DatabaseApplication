...
namespace DatabaseGUI.ExternalWindow.EmployeeWindows {
    ...
    public partial class EmployeeAddressWindow : Window {
        ...
        public EmployeeAddressWindow(Employee employee) {
            InitializeComponent();
            this._employee = employee;
            this.GetData();

            this.AddressListViewer.PopulateList(this.GetAddressesAssociatedWithEmployee(this._employeeAddressList,
                this._addressList));
        ...
        private void ShowResidence() {
            this.GetData();
            var residenceList =
                this._employeeAddressList.Where(b => b.Bsn == this._employee.Bsn && b.IsResidence)
                    .Select(a => new {a.PostalCode, a.Country})
                    .ToList();

            if (residenceList.Count < 1) {
                this.CurrentResidence.Content =
                    $"{this._employee.Name} {this._employee.Surname} does not have a residence";
                return;
            }

            var residence =
                this._addressList.Where(
                    a => a.PostalCode == residenceList[0].PostalCode && a.Country == residenceList[0].Country)
